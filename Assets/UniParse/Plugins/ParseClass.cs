...
public class ParseClass
{
	...
	public static ParseInstance Authenticate (string username, string password)
	{
		...
		var queryString = HTTP.URL.Encode ("username=" + username) + "&" + HTTP.URL.Encode ("password=" + password);
		var r = UniParse.Instance.Request ("GET", "/login" + "?" + queryString);
...
public class ParseInstance
{
	...
	public void Delete ()
	{
		...
		isDone = false;
		var r = UniParse.Instance.Request ("DELETE", string.Format ("{0}/{1}", parseClass.root, objectId));
		r.Send (delegate(HTTP.Request request) {
	...
	public void Update ()
	{
		...
		foreach (var i in dirtyFields)
			payLoad[i] = json[i];
		dirtyFields.Clear ();
		var r = UniParse.Instance.Request ("PUT", string.Format ("{0}/{1}", parseClass.root, objectId), payLoad);
		r.Send (delegate(HTTP.Request request) {
			code = request.response.status;
			Hashtable j = null;
			try {
	...
	public void Create ()
	{
		...
		isDone = false;
		var r = UniParse.Instance.Request ("POST", string.Format ("{0}", parseClass.root), json);
		r.Send (delegate(HTTP.Request request) {
			code = request.response.status;
